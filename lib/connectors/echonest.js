...
var apiActions = {
    ...
    "artistProfile" : { action: ['artist','profile'], in_param_names: ['id','name'], output_nodes: ['response.artist.id', 'response.artist.name'], optionals: [inParamTypesArtist,foreignKeyBucket,'results','start','bucket'] },
    ...
    "artistSearch" : { action: ['artist','search'], in_param_names: ['name'], output_nodes: ['response.artists.artist.id','response.artists.artist.name'], 
        optionals: [foreignKeyBucket,'results','start','bucket',limit,'description','style','mood', { paramName: "rank_type", paramValues: ["relevance", "familiarity"] }, { paramName: "fuzzy_match", paramValues: ["true", "false"] },
    ...
    "artistExtract" : { action: ['artist','extract'], in_param_names: ['text'], output_nodes: ['response.artists.artist.id','response.artists.artist.name'], optionals: [foreignKeyBucket,'results','start','bucket','max_familiarity','min_familiarity','max_hotttnesss','min_hotttnesss',limit,sort] },
    ...
    "artistSimilar" : { action: ['artist','similar'], in_param_names: ['id','name'], output_nodes: ['response.artists.id', 'response.artists.name'], optionals: [inParamTypesArtist,foreignKeyBucket,'results','min_results','start','bucket','max_familiarity','min_familiarity','max_hotttnesss','min_hotttnesss', "artist_start_year_before", "artist_start_year_after", "artist_end_year_before", "artist_end_year_after", limit] },
    ...
    "artistTopHot" : { action: ['artist','top_hottt'], in_param_names: [], output_nodes: ['response.artists.id'] , optionals: [inParamTypesArtist,foreignKeyBucket,'results','start','bucket',limit] },
    ...
    output_nodes: ['response.songs.artist_id', 'response.songs.id', 'response.songs.artist_name', 'response.songs.title'], optionals: [foreignKeyBucket,'results','start','bucket','min_tempo','max_tempo','min_duration','max_duration','min_loudness','max_loudness', 'min_danceability','max_danceability', 'min_energy','max_energy', 'mode', 'key', 'description','style','mood',limit, songSort] },
    
    "songProfile" : { action: ['song','profile'], in_param_names: ['id','track_id'], output_nodes: ['response.songs.artist_id', 'response.songs.id', 'response.songs.artist_name', 'response.songs.title'], optionals: [foreignKeyBucket, inParamTypesTrack,'results','start','bucket', limit] }
};
