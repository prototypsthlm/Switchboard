...
BaseConnector.prototype.execute = function(queryData, apiConfig, cb) {
    ...
    async.forEach(queryData, function fetchData(queryDataItem, seriesDone){
        ...
        async.forEach(queryDataItem.queries, function getResults(query, done) {
            ...
            self.get(url, query, apiConfig, index, queryDataItem.callIndex, done);
            index += 1;
        },
        function(err){
            seriesDone();
...
*/
BaseConnector.prototype.get = function(url, query, apiConfig, index, callIndex, done) {
    var self = this;
    request({ url: url, headers: { "Accept" : "application/json" } }, function (error, response, result) {
      if (!error && response.statusCode == 200) {
        ...
        self.responseArray.push(self.connectorResult({query: query, index: index, callIndex: callIndex, apiConfig: apiConfig, result: JSON.parse(result)}));
      }
      else {
          if(response){
