...
var apiActions = {
    ...
    "artistLookup" : { action: ['lookup','artist'], in_param_names: ['uri'], output_nodes: ['artist.albums.album.name','artist.albums.album.href'], statics: [ { paramName : "extras", paramValue: 'albumdetail'} ] },
    "albumLookup" : { action: ['lookup','album'], in_param_names: ['uri'], output_nodes: ['album.name','album.tracks.href','album.tracks.name'], statics: [ { paramName : "extras", paramValue: 'trackdetail'} ] },
...
Spotify.prototype.connectorResult = function(resultObj) {
    var cleanResult = resultObj.result;
    if("info" in resultObj.result) {
        delete cleanResult["info"];
    }
    return new ConnectorResponse(resultObj, cleanResult);
}
