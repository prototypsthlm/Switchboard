...
function ResultFormatter(result, config) {    
    ...
    */
    var setObjectData = function(obj, keys, value) {        
        //console.log("setObjectData(obj, keys, value) ", obj, keys, value);
        if(obj == undefined) {
            console.warn("Object is undefined. Doing nothing");
            return obj;
        }
        if(keys.length > 1) {
        ...
        if(keys[0] == undefined) {
            console.log("Replacing whole object with value");
            obj = value;
        }
        else {
    ...
    */
    ...
    var removeUnusedResult = function(connectorResult, inSource, indexToRemove) {
        ...
        console.log("pathToArray: ", pathToArray);
        if(pathToArray) {
            ...
            if(insourceArray.length) {
                console.log("The key",keyHoldingArray,path);
                ...
                if(resultHoldingArray instanceof Array) {
                    for(var i in resultHoldingArray) {
                        resultHoldingArray[i][keyHoldingArray].splice(indexToRemove,1);
                    }
                } else {
            ...
            if(nextConnectorResults) {
                ...
                //for(var j = 0; j < results.length; j++) { 
                    ...
                    if(nextConnectorResult) {
                        ...
                        console.log("Result retrieved from the next connector. Result going to be merged in connector "+connectorIndex+", result at index "+j);
                        ...
                        console.warn("No result found in connector",connectorIndex+1," at index", j,"using callindex",connectorCall.meta.index,". Remove! (extract merge)!");
