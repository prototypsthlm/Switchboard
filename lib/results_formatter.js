...
var logger = require('./sb_tracer.js')('all');
...
function ResultFormatter(result, config) {    
    ...
    this.findConnectorResult = function(connectorIndex, callIndex, index, returnOnlyResult) {
        ...
        if(!rawResults[connectorIndex]) {
            logger.warn("No rawResults found for index "+connectorIndex);
            return undefined;
        }
        ...
        for(var i in connectorResults.calls) {
            ...
            if(nextConnectorResults) {
                ...
                for(var j = 0; j < results.length; j++) { 
                    ...
                    logger.trace("I am in connector "+connectorIndex+", result at index "+j);  
                    ...
                    if(nextConnectorResult) {
                        ...
                        logger.log("I have retrieved connector result in connector "+connectorIndex+", result at index "+j, nextConnectorResult.result);
                        ...
                        if(arrayInSource) {
                            logger.trace(queryKey+" is array!");
                        ...
                        logger.trace("Added result to index ",newResults.length-1+" in connector",connectorIndex+", index:",index," result index",j);
                    } else if(this.config("mergeMethod") == "inject") {
                        newResults.push(results[j]);
                    } else {
                        logger.warn("No result found in connector",connectorIndex+1," at index", j,"using callindex",connectorCall.meta.index);
                        break;
                    }
                }
